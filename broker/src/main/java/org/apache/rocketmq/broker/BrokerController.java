...
public class BrokerController {
    ...
    public boolean initialize() throws CloneNotSupportedException {
        // 加载磁盘上的配置信息，这些配置信息用到了MessageStoreConfig
        ...
        // P2 Broker的Netty组件，注意，Broker即是服务端(接收Producer和Consumer的请求)，又是客户端(要往NameServer和Producer写入)
        if (result) {
            // Netty 网络组件
            ...
            // 使用的Java的SPI机制加载的服务，意味着可拓展，找某个接口的所有实现类
            initialTransaction();
            initialAcl();
            initialRpcHooks();
        }
    ...
    //P1 BrokerController.start()
    public void start() throws Exception {
        // 启动核心消息组件
        if (this.messageStore != null) {
            this.messageStore.start();
        }
        //P2 Broker启动了两个Netty服务，这样就可以接收请求了
        if (this.remotingServer != null) {
        ...
        //P2 Broker的brokerOuterAPI可以理解为一个Netty客户端，往外发送请求，例如心跳。
        if (this.brokerOuterAPI != null) {
